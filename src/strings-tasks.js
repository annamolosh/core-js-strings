...
function isPalindrome(str) {
  ...
  const clearStroke = palindrome.split('').reverse().join('');
  return palindrome === clearStroke;
}
...
function reverseWords(str) {
  ...
  const revesedWords = words.map((word) => {
    return word.split('').reverse().join('');
  });
  return revesedWords.join(' ');
}
...
function invertCase(str) {
  const characters = str.split('').map((char) => {
...
function getCardId(value) {
  ...
  const cardValue = value.slice(0, -1).replace('10', 'T');
